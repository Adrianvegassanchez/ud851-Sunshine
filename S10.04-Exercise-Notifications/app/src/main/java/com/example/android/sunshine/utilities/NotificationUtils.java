...
//  TODODONE (1) Create a constant int value to identify the notification
...
//          TODODONE (2) Use NotificationCompat.Builder to begin building the notification
...
//          TODODONE (3) Create an Intent with the proper URI to start the DetailActivity
            Intent detailIntentForToday = new Intent(context, DetailActivity.class);
            detailIntentForToday.setData(todaysWeatherUri);

//          TODODONE (4) Use TaskStackBuilder to create the proper PendingIntent
            TaskStackBuilder taskStackBuilder = TaskStackBuilder.create(context);
            taskStackBuilder.addNextIntentWithParentStack(detailIntentForToday);
            PendingIntent resultPendingIntent = taskStackBuilder
                    .getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT);

//          TODODONE (5) Set the content Intent of the NotificationBuilder
            notificationBuilder.setContentIntent(resultPendingIntent);

//          TODODONE (6) Get a reference to the NotificationManager
            NotificationManager notificationManager = (NotificationManager)
                    context.getSystemService(Context.NOTIFICATION_SERVICE);

//          TODODONE (7) Notify the user with the ID WEATHER_NOTIFICATION_ID
            notificationManager.notify(WEATHER_NOTIFICATION_ID, notificationBuilder.build());

//          TODODONE (8) Save the time at which the notification occurred using SunshinePreferences
